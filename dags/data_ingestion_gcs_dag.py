...
dataset_file_csv = "nation_population.csv"
dataset_file_json = "nation_population.json"
...
BIGQUERY_DATASET = os.environ.get("BIGQUERY_DATASET", 'nation_population')

def call_dataset(src_file_json):
    data_json = requests.get(dataset_url).content
    data_json = json.loads(data_json)
    with open(src_file_json, 'w') as file:
        json.dump(data_json, file)

def save_as_csv(src_file_json, src_file):
    if not src_file_json.endswith('.json'):
        logging.error("Can only accept source files in JSON format, for the moment")
        return
    with open(src_file_json, "r") as file:
        data_json = json.load(file)
        df = pd.json_normalize(data_json, 'data').to_csv(src_file, index=False)
...
with DAG(
    ...
    call_dataset_task = PythonOperator(
        ...
        op_kwargs={
            "src_file_json": f"{path_to_local_home}/{dataset_file_json}",
        },
    )
